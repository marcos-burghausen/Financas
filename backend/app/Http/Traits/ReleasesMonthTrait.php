...
trait ReleasesMonthTrait
{
    ...
    public function classifiesReleases(iterable $lacamentos, string $tipoLancamentos, $date = null): array
    {
        $releasesMonth = $lacamentos;
    ...
    // A função lancamentosMes() não é mais chamada por classifiesReleases, mas pode ser usada por outras partes do código.
    public function lancamentosMes(object $releases, $date): array
    {
        $mes = substr($date, 5, 2);
        $ano = substr($date, 0, 4);
        $lancamentosMes = [];
        foreach ($releases as $release) {
            if (substr($release->data_vencimento, 5, 2) === $mes && substr($release->data_vencimento, 0, 4) === $ano) {
                $lancamentosMes[] = $release;
            }
        }
    ...
    public function totalExpensesDay(iterable $releasesMonth): int
    {
        $totalExpensesDay = 0;
    ...
    public function totalByCategory(iterable $releasesMonth): array
    {
        $totalByCategory = [];
        foreach ($releasesMonth as $release) {
    ...
    {
        $dataInicio = Carbon::parse($mes)->startOfMonth();
        $dataFim = Carbon::parse($mes)->endOfMonth();
