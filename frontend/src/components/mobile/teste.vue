<template>
  <div class="container__modal">
    ...
    >
      ...
      <!-- Modal para selecionar o tipo de lançamento -->
      <div v-if="openTipoLancamento" class="tipo">
        <div class="modal__tipo">
          <div v-for="(item, index) in tiposLancamento" :key="index" class="cor__icon">
      ...
      <div v-if="openParcelas" class="parcelas">
        <div class="container__parcelas pb-5">
          ...
          <div class="botoes__parcelas mx-5">
            <v-btn class="px-5 me-5 cancelar" @click="openParcelas = false">
              Cancelar
            </v-btn>
            <v-btn class="btn__concluido px-5" @click="openParcelas = false">
              Concluído
            </v-btn>
          </div>
        </div>
      ...
      >
        <template #prepend-inner>
          <mdicon class="icon__modify" :name="formReleases.status === 'Efetivada' ? 'check-circle-outline' : 'clock-time-three-outline'" />
        </template>
        <template #append-inner>
          <div :class="formReleases.status === 'Efetivada' ? 'form__check__efetivada' : 'form__check'">
            <div :class="formReleases.status === 'Efetivada' ? 'switch__check__efetivada' : 'switch__check'" />
          </div>
        </template>
      </v-text-field>
...
const contas = ref(
  useWallets.walletsData && useWallets.walletsData.wallets
...
const rules = {
  requiredValor: (value: string) => !!value || "O campo
